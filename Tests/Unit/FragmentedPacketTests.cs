...
public class FragmentedPacketTests
{
    ...
    public void Adding_negative_fragment_number_throws() =>
        Assert.That(() => _fragmentedPacket.Add(Packet.Get(), fragmentNumber: -1, isLastFragment: true), Throws.Exception);

    [Test]
    public void Adding_one_and_last_fragment_should_produce_reassembled_packet()
    {
        _fragmentedPacket.Add(Packet.Get(), fragmentNumber: 0, isLastFragment: true);
    ...
    {
        var isFirstAdded = _fragmentedPacket.Add(Packet.Get(), fragmentNumber: 0, isLastFragment: true);
        var isSecondAdded = _fragmentedPacket.Add(Packet.Get(), fragmentNumber: 0, isLastFragment: true);
